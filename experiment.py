...
def experiment(
    ...
    # load dataset
    dataset_path = f'/home/lxy/tsj/pgp/D4RL/offline_MLP/{env_name}-{dataset}-v{dversion}.pkl'  #MLP dataset
    ...
    if variant['norm_r']:
        for path in trajectories:
            path['rewards'] = path['rewards_ensembles']

    # save all path information into separate lists
    mode = variant.get('mode', 'normal')
    states, traj_lens, returns, rewards, uncertainties = [], [], [], [], []
    for path in trajectories:
    ...
    def eval_episodes(model, critic):
        returns, lengths = [], []
        for _ in range(num_eval_episodes):
            with torch.no_grad():
                ret, length = evaluate_episode_rtg(
                    ...
                    target_return = [0.5 * max_return/variant['test_scale'],0.75 * max_return/variant['test_scale'],max_return/variant['test_scale'],1.5*max_return/variant['test_scale'],2*max_return/variant['test_scale']],
                    mode=mode,
        ...
        return {
            ...
            f'normalized_score': env.get_normalized_score(np.mean(returns)),
        }
    ...
    for iter in range(variant['max_iters']):
        outputs = trainer.train_iteration(num_steps=variant['num_steps_per_iter'], logger=logger, 
                    iter_num=iter+1, log_writer=None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='pen')
    ...
    parser.add_argument("--grad_norm", default=9.0, type=float)
    ...
    parser.add_argument("--norm_r", action='store_true', default=False)
