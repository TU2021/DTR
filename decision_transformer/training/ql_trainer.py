...
class Trainer:
    ...
    def __init__(self, 
        ...
        if hasattr(self.actor, 'log_temperature'):
            self.log_temperature_optimizer = torch.optim.Adam(
                [self.actor.log_temperature],
                lr=1e-4,
                betas=[0.9, 0.999],
            )
        ...
        self.eval_fns = None if eval_fns is None else eval_fns
    ...
    def train_iteration(self, num_steps, logger, iter_num=0, log_writer=None):
        ...
        for _ in trange(num_steps):
            loss_metric = self.train_step(num_steps, log_writer, loss_metric)
        ...
        outputs = self.eval_fns(self.actor, self.critic_target)
    ...
    def linear_decay_coefficient(self, step, max_steps, initial_coefficient=0, min_coefficient=1.0):
        change_per_step = (initial_coefficient - min_coefficient) / max_steps
        coefficient = initial_coefficient - change_per_step * step
        return coefficient

    def train_step(self, num_steps, log_writer=None, loss_metric={}):
        '''
        ...
        coeff_eta = self.linear_decay_coefficient(self.step, self.maxt * num_steps, 0, 1.0)
        ...
        actor_loss = self.eta2 * bc_loss + self.eta * coeff_eta * q_loss
