...
def evaluate_episode_rtg(
    ...
    if len(ep_return) > 1:
        ...
        ep_return = torch.tensor(ep_return, device=device, dtype=torch.float32).unsqueeze(-1)
    else:
        target_return = torch.tensor(ep_return, device=device, dtype=torch.float32).reshape(1, 1)
        ep_return = torch.tensor(ep_return, device=device, dtype=torch.float32).reshape(1, 1)
    timesteps = torch.tensor(0, device=device, dtype=torch.long).reshape(1, 1)
    rewards = [torch.zeros((1, 1), device=device, dtype=torch.float32)]
    ...
    with torch.no_grad():
        for t in range(max_ep_len):
            ...
            state_numpy = states[-1:,:].detach().cpu().numpy()
            ...
            if mode != 'delayed':
                ...
                if not reward_model:
                    pred_return = target_return[:,-1:] - (ep_return/max_ep_len)
                else:
                    obs_act = np.concatenate([state_numpy, action_numpy], axis=-1)
                    reward_r, reward_r_std = reward_model.get_reward_batch_ensemble(obs_act)
                    pred_return = target_return[:,-1:] - torch.from_numpy(reward_r/scale).to(device=device)
