...
from utils import reward_from_preference
...
def load_queries_with_indices(dataset, num_query, len_query, saved_indices, saved_labels=None, label_type=1,
    ...
    percentage = 1.0
    top_n = int(len(next(iter(batch.values()))) * percentage)
    batch =  {k: v[:top_n] for k, v in batch.items()}
...
def train(cfg):
    ...
    if cfg.fake_label:
        ...
        suffix = 'human_labels'
        if cfg.domain in ["robosuite"]:
            data_dir = os.path.join(cfg.data_dir, f"{cfg.env}_{cfg.robosuite_dataset_type}_{suffix}")
        else:
    ...
    if os.path.exists(data_dir):
        if cfg.domain in ["robosuite"]:
            suffix = f"num{cfg.num_query}_q{cfg.len_query}"
            # suffix = f"domain_{cfg.domain}_env_{cfg.env}_{cfg.robosuite_dataset_type}_num_{cfg.num_query}_len_{cfg.len_query}"
        else:
    ...
    if cfg.modality == "state":
        ...
        reward_model.load_model(cfg.reward_model_path)

        dataset_relabel = reward_from_preference(dataset, reward_model, reward_model_type=cfg.structure, device="cuda")
    
        with open(f'{cfg.pkl_path}.pkl', 'wb') as f: 
            pickle.dump(dataset_relabel, f)
