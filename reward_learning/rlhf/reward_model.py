...
class RewardModel(object):
    ...
    def get_reward_batch(self, x):
        ...
        return np.mean(r_hats, axis=0), np.std(r_hats, axis=0), np.min(r_hats, axis=0)

    def get_reward_batch_ensemble(self, x):
        # they say they average the rewards from each member of the ensemble,
        # but I think this only makes sense if the rewards are already normalized.
        # but I don't understand how the normalization should be happening right now :(
        r_hats = []
        ...
        return np.mean(r_hats, axis=0), r_hats
    ...
    def predict_with_uncertainty(self, x, n_samples=10):
        ...
        for _ in range(n_samples):
            preds = []
            for member in range(self.ensemble_size):
                preds.append(self.r_hat_member(x, member=member).detach().cpu().numpy())
        ...
        return np.mean(mean_prediction, axis=0), np.mean(uncertainty, axis=0)

    def enable_dropout(self, enable=True):
        for model in self.ensemble:
            for layer in model.modules():
                if isinstance(layer, nn.Dropout):
                    layer.train() if enable else layer.eval()
